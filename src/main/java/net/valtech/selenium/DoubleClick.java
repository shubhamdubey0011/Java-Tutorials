...
public class DoubleClick {
	...
	public void doubleClick() throws InterruptedException {
		...
		// "D:\\Selenium Environment\\Drivers\\chromedriver.exe");
		...
		// As per the above URL we need to switch to frame. The targeted element
		// is in the frame
		...
		// Here I used JavascriptExecutor interface to scroll down to the
		// targeted element
		((JavascriptExecutor) driver).executeScript(
				"arguments[0].scrollIntoView();", ele);
		// used doubleClick(element) method to do double click action
		action.doubleClick(ele).build().perform();
		// Once clicked on the element, the color of element is changed to
		// yellow color from blue color
		// driver.close();
	}
}
